...
static CRGB DimColor(uint32_t color)
{
	...
	return FastLedTask::Color(Red(color) >> 1, Green(color) >> 1, Blue(color) >> 1);
}

// Input a value 0 to 255 to get a color value.
// The colors are a transition r - g - b - back to r.
/*static*/ uint32_t FastLedTask::Wheel(byte WheelPos, SongModeT songMode)
{
...
FastLedTask::FastLedTask(CLEDController& controller, 
	...
	, RecurringTask(0 /*intervalMs*/, liftimeMs)
{
	mRuntimeMs= 4000;
